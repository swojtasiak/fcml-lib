...
typedef struct fcml_ist_asm_dec_operand_decoding {
	...
	// Calculates size of the encoded operand in bytes.
	fcml_ifp_asm_dec_operand_size_calculator size_calculator;
	// Optional hints, if operand has any.
	fcml_hints hints;
} fcml_ist_asm_dec_operand_decoding;

typedef struct fcml_st_asm_dec_modrm_decoding_details {
	fcml_bool is_vsib;
	fcml_data_size vsib_index_size;
} fcml_st_asm_dec_modrm_decoding_details;
...
typedef struct fcml_ceh_asm_dec_instruction_decoding_def {
	...
	fcml_hints hints;
	// ModRM details.
	fcml_st_asm_dec_modrm_decoding_details modrm_details;
...
void fcml_ifn_asm_dec_utils_set_x64_exp( fcml_st_register *reg, fcml_bool is_rex ) {
	if( is_rex ) {
...
fcml_ceh_error fcml_fnp_asm_dec_operand_decoder_imm( fcml_st_asm_decoding_context *context, fcml_ist_asm_dec_operand_wrapper *operand_wrapper, fcml_ptr args ) {
	...
	fcml_data_size imm_size = fcml_ifn_dec_asm_decode_encoded_size_value( context, imm_args->encoded_imm_size );
	fcml_data_size imm_size_ex = fcml_ifn_dec_asm_decode_encoded_size_value( context, imm_args->encoded_ex_imm_size );
...
fcml_ceh_error fcml_fnp_asm_dec_operand_decoder_rm( fcml_st_asm_decoding_context *context, fcml_ist_asm_dec_operand_wrapper *operand_wrapper, fcml_ptr args ) {
	...
	if( ( rm_args->flags & FCML_RMF_R ) && decoded_modrm->reg.is_not_null ) {
		...
		fcml_ifn_asm_dec_utils_set_x64_exp( &(operand->reg), context->prefixes.is_rex );
		...
		*address = decoded_modrm->address;
		...
		// TODO: odkomentowac po zakonczeniu testow.
		//error = FCML_EN_UNSUPPORTED_ADDRESSING_MODE;
	}
...
fcml_ceh_error fcml_fnp_asm_dec_operand_decoder_r( fcml_st_asm_decoding_context *context, fcml_ist_asm_dec_operand_wrapper *operand_wrapper, fcml_ptr args ) {
	...
	reg->size = fcml_ifn_dec_asm_decode_encoded_size_value( context, r_args->encoded_register_operand_size );

	fcml_ifn_asm_dec_utils_set_x64_exp( reg, context->prefixes.is_rex );
...
fcml_ceh_error fcml_fnp_asm_dec_operand_decoder_vex_vvvv( fcml_st_asm_decoding_context *context, fcml_ist_asm_dec_operand_wrapper *operand_wrapper, fcml_ptr args ) {
	...
	reg->size = fcml_ifn_dec_asm_decode_encoded_size_value( context, v_args->encoded_register_size );

	return error;
}

fcml_ceh_error fcml_fnp_asm_dec_operand_decoder_isX( fcml_st_asm_decoding_context *context, fcml_ist_asm_dec_operand_wrapper *operand_wrapper, fcml_ptr args ) {
...
fcml_int fcml_fnp_asm_dec_operand_size_calculator_isX( fcml_st_asm_decoding_context *context, fcml_ptr args ) {
	return 1;
}
...
fcml_st_asm_dec_operand_decoder_def fcml_def_operand_decoders[] = {
	...
	{ fcml_fnp_asm_dec_operand_decoder_isX, fcml_fnp_asm_dec_operand_size_calculator_isX, NULL },
...
void fcml_fn_asm_dec_dts_prepare_modrm_decoding_details( fcml_st_def_decoded_addr_mode *decoded_addr_mode, fcml_st_asm_dec_modrm_decoding_details *modrm_details ) {
...
fcml_ceh_error fcml_ifn_asm_dec_dts_prepare_instruction_decoding_callback_default( fcml_st_dialect_context *dialect, fcml_ceh_asm_dec_tree_diss_tree_element *element, fcml_st_def_instruction_description *instruction_desc, fcml_st_def_addr_mode_desc *addr_mode_desc ) {
	...
	for( i = 0; i < FCML_OPERANDS_COUNT; i++ ) {
		...
		if( !error ) {
			fcml_fn_asm_dec_dts_prepare_modrm_decoding_details( decoding->operand_decodings[i].decoded_addr_mode, &(decoding->modrm_details) );
		} else {
			fcml_ifn_asm_dec_dts_free_acceptors_chain( decoding->instruction_acceptors_chain );
			fcml_fn_mp_free_mnemonics( decoding->mnemonics );
			fcml_fn_env_memory_free( decoding );
...
fcml_ceh_error fcml_fn_asm_dec_instruction_decoder_IA( fcml_st_asm_decoding_context *decoding_context, fcml_ceh_asm_dec_instruction_decoding_def *instruction_decoding_def ) {
	...
	fcml_st_asm_dec_modrm_decoding_details *modrm_details = &(instruction_decoding_def->modrm_details);
