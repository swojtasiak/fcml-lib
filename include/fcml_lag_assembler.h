 ...
 * Assembles all instructions given in the NULL terminated source_code array of strings.
 * Every instruction has to be represented as one string in the source array. Like every multipass
 * assembler implementation it passes through the source code multiple times in order to generate
 * optimal code. Assembler also supports symbols that can be provided using symbol table
 * available in the context. Context should be initialized the same way as in case of one line assembler.
 * The only difference here is the symbol table itself which can be initialized using fcml_fn_symbol_table_alloc()
 * function. Symbols can be also declared directly in the source code and accessed
 * through the same symbols table when processing is done (Only if you provide valid symbol table through the context).
 * Reusable result holder has to be prepared using fcml_fn_lag_assembler_result_prepare() function. As long as the context
 * and the result holder are not shared across multiple calls function is thread safe.
 *
 ...
 * @warning This is an experimental implementation and it still needs some testing.
 ...
 * Every instance of fcml_st_lag_assembler_result structure is reusable from the assembler's
