...
fcml_coll_map fcml_fn_coll_map_alloc_factor( const fcml_st_coll_map_descriptor *descriptor, fcml_uint32_t capacity, float load_factor, fcml_int *error ) {
    ...
    *error = FCML_COLL_ERROR_NO_ERROR;
    ...
    struct fcml_ist_coll_map *map = (struct fcml_ist_coll_map*) fcml_fn_env_memory_alloc( sizeof(struct fcml_ist_coll_map) );
    if (map == NULL) {
...
fcml_bool fcml_ifn_coll_map_expand( struct fcml_ist_coll_map *map, fcml_ceh_error *error ) {
    ...
    *error = FCML_COLL_ERROR_NO_ERROR;
    ...
    struct fcml_ist_coll_map_entry** new_map_entries = (struct fcml_ist_coll_map_entry**) fcml_fn_env_memory_alloc(
...
void fcml_fn_coll_map_put( fcml_coll_map map_int, const fcml_ptr key, const fcml_ptr value, fcml_int *error ) {
    ...
    *error = FCML_COLL_ERROR_NO_ERROR;
    ...
    struct fcml_ist_coll_map_entry *newEntry = (struct fcml_ist_coll_map_entry*) fcml_fn_env_memory_alloc( sizeof(struct fcml_ist_coll_map_entry) );
    if (!newEntry) {
        *error = FCML_COLL_ERROR_OUT_OF_MEMORY;
        return;
    }
...
void fcml_fn_coll_map_iterate( const fcml_coll_map map_int, fcml_fnp_coll_map_entry_handler item_handler ) {
    if (item_handler) {
        struct fcml_ist_coll_map *map = (struct fcml_ist_coll_map*) map_int;
        fcml_uint32_t i;
        for ( i = 0; i < map->capacity; i++ ) {
