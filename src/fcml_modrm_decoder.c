...
fcml_ceh_error fcml_ifn_modrm_decode_displacement( fcml_st_memory_stream *stream, fcml_st_integer *displacement, fcml_st_offset *offset, fcml_usize size,
        fcml_usize offset_extension_size, fcml_usize displacement_extension_size ) {
...
fcml_ceh_error fcml_ifn_modrm_decode_16bit( fcml_st_modrm_decoder_context *context, fcml_st_modrm_source *modrm_source, fcml_st_modrm *decoded_modrm,
    ...
    if ( f_mod == 0 && f_rm == 6 ) {
        ...
        /* disp16.*/
        fcml_ceh_error error = fcml_ifn_modrm_decode_displacement( stream, &( effective_address->displacement ), &( address->offset ), FCML_DS_16,
                context->effective_address_size, fcml_ifn_modrm_get_displacement_extension( context, flags ) );
        if ( error ) {
            return error;
        }
        ...
        if ( f_mod > 0 ) {
            fcml_ceh_error error = fcml_ifn_modrm_decode_displacement( stream, &( effective_address->displacement ), NULL,
                    ( f_mod == 1 ) ? FCML_DS_8 : FCML_DS_16, 0, fcml_ifn_modrm_get_displacement_extension( context, flags ) );
            if ( error ) {
                return error;
            }
        }
...
fcml_ceh_error fcml_ifn_modrm_decode_sib( fcml_st_modrm_decoder_context *context, fcml_uint8_t mod_rm, fcml_st_modrm_source *modrm_source,
        fcml_st_modrm *decoded_modrm, fcml_st_modrm_details *modrm_details, fcml_uint8_t flags ) {
    ...
    if ( f_mod == 0 && FCML_MODRM_SIB_BASE( sib ) == 5 ) {
        ...
        if ( f_mod > 0 ) {
            error = fcml_ifn_modrm_decode_displacement( stream, &( effective_address->displacement ), NULL, ( f_mod == 1 ) ? FCML_DS_8 : FCML_DS_32, 0,
...
fcml_ceh_error fcml_ifn_modrm_decode_3264bit( fcml_st_modrm_decoder_context *context, fcml_st_modrm_source *modrm_source, fcml_st_modrm *decoded_modrm,
        fcml_st_modrm_details *modrm_details, fcml_uint8_t flags ) {
    ...
    if ( f_mod == 3 ) {
        ...
        error = fcml_ifn_modrm_decode_displacement( stream, &( effective_address->displacement ), &( address->offset ), FCML_DS_32,
                decoded_modrm->is_rip ? 0 : effective_address_size, 0 );
        if ( error ) {
            return error;
        }
