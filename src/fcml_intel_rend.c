...
static fcml_string integer_formats[6][4] = {
        ...
        /* Signed hex values.*/
        {
                FCML_PRI_INT8_HEX FCML_TEXT("h"),
                FCML_PRI_INT16_HEX FCML_TEXT("h"),
                FCML_PRI_INT32_HEX FCML_TEXT("h"),
                FCML_PRI_INT64_HEX FCML_TEXT("h")
        },
        /* Unsigned hex values.*/
        {
                FCML_PRI_INT8_HEX FCML_TEXT("h"),
        ...
        {
                FCML_PRI_INT8_HEX_NO_ZEROS FCML_TEXT("h"),
                FCML_PRI_INT16_HEX_NO_ZEROS FCML_TEXT("h"),
                FCML_PRI_INT32_HEX_NO_ZEROS FCML_TEXT("h"),
                FCML_PRI_INT64_HEX_NO_ZEROS FCML_TEXT("h")
        },
...
static fcml_ceh_error operand_renderer_imm(
    ...
    return fcml_fn_rend_utils_format_append_integer(
            integer_formats, output_stream,
            immediate, config->render_flags & FCML_REND_FLAG_HEX_IMM,
            config->render_flags & FCML_REND_FLAG_REMOVE_LEADING_ZEROS);
}

static fcml_ceh_error operand_renderer_reg(
...
static fcml_ceh_error rend_size_operator(fcml_usize size_operator,
        fcml_string buffer, fcml_usize buffer_len,
        fcml_bool is_media_instruction) {
    ...
    case 48:
        ...
        break;
    case 64:
        size_operator_printable = is_media_instruction ?
                        FCML_TEXT("mmword ptr ") : FCML_TEXT("qword ptr ");
        break;
    case 80:
    ...
    case 128:
        size_operator_printable = is_media_instruction ?
                        FCML_TEXT("xmmword ptr ") : FCML_TEXT("oword ptr ");
        break;
    case 256:
        size_operator_printable = is_media_instruction ?
                        FCML_TEXT("ymmword ptr ") : FCML_TEXT("qqword ");
        break;
    case 512:
        size_operator_printable = FCML_TEXT("zmmword ptr ");
        break;
    default:
        fcml_fn_env_str_snprintf(buffer, buffer_len, FCML_TEXT("%dbyte ptr "),
                size_operator / 8);
        break;
    }
...
static fcml_ceh_error operand_renderer_address(
    ...
    if (address->size_operator > 0) {
        ...
        rend_size_operator(address->size_operator, buffer, sizeof(buffer),
                operand->hints & FCML_OP_HINT_MULTIMEDIA_INSTRUCTION);
        fcml_fn_rend_utils_format_append_str(output_stream, buffer);
    }

    if (!address->segment_selector.is_default_reg
            || (render_flags & FCML_REND_FLAG_RENDER_DEFAULT_SEG)) {
        fcml_fn_rend_utils_format_append_reg(dialect_context, output_stream,
    ...
    if ((render_flags & FCML_REND_FLAG_RENDER_SIB_HINT)
            && (result->instruction_details.modrm_details.sib.is_not_null)) {
        fcml_fn_rend_utils_format_append_str(output_stream, FCML_TEXT("sib "));
    }

    if ((render_flags & FCML_REND_FLAG_RENDER_ABS_HINT)
            && (operand->hints & FCML_OP_HINT_ABSOLUTE_ADDRESSING)) {
        fcml_fn_rend_utils_format_append_str(output_stream, FCML_TEXT("abs "));
    }

    if ((render_flags & FCML_REND_FLAG_RENDER_REL_HINT)
    ...
    if (address->address_form == FCML_AF_COMBINED) {
        ...
        if (!is_reg_undef(&(effective_address->index))) {
            fcml_fn_rend_utils_format_append_str_if(output_stream,
                    FCML_TEXT("+"), !first);
            fcml_fn_rend_utils_format_append_reg(dialect_context, output_stream,
        ...
        if (effective_address->displacement.size > 0) {
            ...
            if ((render_flags & FCML_REND_FLAG_HEX_DISPLACEMENT)
                    || !fcml_fn_utils_is_disp_neg(
...
static fcml_ceh_error operand_renderer_far_pointer(
    ...
    fcml_fn_rend_utils_format_append_integer(
            integer_formats, output_stream, &integer,
            FCML_TRUE,
            config->render_flags & FCML_REND_FLAG_REMOVE_LEADING_ZEROS);
...
static fcml_ceh_error print_operand(const fcml_st_render_config *config,
    ...
    case FCML_OT_ADDRESS:
        error = operand_renderer_address(config,
                dialect_context, output_stream, result, operand);
        break;
    case FCML_OT_REGISTER:
        error = operand_renderer_reg(dialect_context, output_stream, result,
                operand);
        break;
...
static fcml_string resolve_mnemonic(const fcml_st_disassembler_result *result) {
...
fcml_ceh_error fcml_fn_rend_render_instruction_intel(
    ...
    if ((render_flags & FCML_REND_FLAG_RENDER_INDIRECT_HINT)
            && (result->instruction.hints & FCML_HINT_INDIRECT_POINTER)) {
        len += fcml_fn_rend_utils_format_append_str(output_stream,
                FCML_TEXT(" indirect"));
    }
    ...
    for (i = 0; i < FCML_OPERANDS_COUNT; i++) {
        if (result->instruction.operands[i].type != FCML_OT_NONE) {
            error = print_operand(config,
                    dialect_context_int, &local_stream, result, i);
            if (!error) {
