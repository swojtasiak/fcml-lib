...
fcml_ceh_error fcml_fn_prepare_entry_point( fcml_st_entry_point *entry_point ) {
    ...
    /* Mode has to be set. */
    if ( entry_point->op_mode != FCML_OM_16_BIT && entry_point->op_mode != FCML_OM_32_BIT && entry_point->op_mode != FCML_OM_64_BIT ) {
        return FCML_CEH_GEC_INVALID_OPERATING_MODE;
    }

    /* 16 bit address size attribute is not supported in 64bit mode. */
    if ( entry_point->op_mode == FCML_OM_64_BIT && entry_point->address_size_attribute == FCML_DS_16 ) {
