...
fcml_ceh_error fcml_fn_parser_add_symbol_to_symbol_table( fcml_st_ceh_error_container *errors, fcml_st_symbol_table symbol_table, fcml_st_symbol *symbol,
    ...
    if ( !allow_override ) {
        if ( fcml_fn_symbol_get( symbol_table, symbol->symbol ) ) {
            ...
            fcml_fn_msg_add_error_message( errors, FCML_MC_SEGMENT_SYMBOL_ALREADY_DEFINED, FCML_CEH_MEC_ERROR_SYMBOL_ALREADY_DEFINED, FCML_EN_CEH_EL_ERROR,
                    symbol->symbol );
    ...
    if ( ast.symbol ) {
        ...
        if ( !context->configuration.disable_symbols_declaration ) {
            ...
            if ( context->symbol_table || context->configuration.alloc_symbol_table_if_needed ) {
                ...
                error = fcml_fn_parser_add_symbol_to_symbol_table( &( result->errors ), context->symbol_table, ast.symbol,
