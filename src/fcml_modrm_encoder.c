...
fcml_ceh_error fcml_ifn_modrm_encode_displacement( fcml_st_modrm_encoder_context *context, const fcml_st_integer *displacement,
        fcml_st_encoded_modrm *encoded_modrm, fcml_usize asa, fcml_usize *disp_size ) {
...
fcml_ceh_error fcml_ifn_modrm_encode_3264bit( fcml_st_modrm_encoder_context *context, const fcml_st_modrm *decoded_modrm, fcml_st_encoded_modrm *encoded_modrm ) {
    ...
    if ( ( context->chosen_effective_address_size == FCML_DS_64 && context->op_mode != FCML_OM_64_BIT )
            || ( context->op_mode == FCML_OM_64_BIT && context->chosen_effective_address_size == FCML_DS_16 ) ) {
        return FCML_CEH_GEC_INVALID_ADDRESSING_FORM;
    }
    ...
    if ( !is_rip_disp ) {
        ...
        if ( is_index ) {
            ...
            if ( !is_base && is_displacement ) {
                ...
                fcml_bool choose_rip = context->choose_rip_encoding && ( context->op_mode == FCML_OM_64_BIT );
                ...
                if ( choose_rip ) {
                    ...
                    choose_sib = ( context->op_mode == FCML_OM_64_BIT ) ? FCML_TRUE : context->choose_sib_encoding;
    ...
    if ( is_rip ) {
        ...
        fcml_usize asa = ( context->chosen_effective_address_size ) ? context->chosen_effective_address_size : context->effective_address_size;
        if ( asa == FCML_DS_32 ) {
        ...
        if ( is_base || is_index ) {
            ...
            fcml_ceh_error error = fcml_ifn_modrm_encode_displacement( context, &( effective_address->displacement ), encoded_modrm,
            ...
            fcml_usize error = fcml_ifn_modrm_convert_absolute_address_to_integer( address, &absolute_address );
            if ( error ) {
                return error;
            }
