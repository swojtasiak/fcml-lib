...
fcml_bool fcml_fn_ts_instruction_test(fcml_uint8_t *code, fcml_int size,
        fcml_en_operating_mode op_mode, fcml_string mnemonic,
        fcml_uint32_t t_flags, fcml_uint32_t rend_flags) {
    ...
    if (error == FCML_CEH_GEC_NO_ERROR) {
        ...
        if (!(t_flags & FCML_TSF_ASM_ONLY)) {
            ...
            for (i = 0; i < FCML_DASM_PREFIXES_COUNT; i++) {
                if (dis_result.instruction_details.prefixes_details.prefixes[i].prefix
                        == 0x67) {
                    is_67 = FCML_TRUE;
                }
                if (dis_result.instruction_details.prefixes_details.prefixes[i].prefix
            ...
            config.render_flags = FCML_REND_FLAG_HEX_IMM
                    ...
                    | FCML_REND_FLAG_HEX_DISPLACEMENT | rend_flags;

            error = fcml_fn_render(dialect, &config, buffer, sizeof(buffer),
                    &dis_result);
            if (error) {
        ...
        fcml_ceh_error error = fcml_fn_parse(&parser_context, mnemonic,
                &result);
        if (error) {
        ...
        context.configuration.choose_abs_encoding = !(t_flags
                & FCML_TSF_ENABLE_RIP);
        ...
        if (error) {
        ...
        } else if (t_flags & FCML_TSF_SHOULD_FAIL) {
            printf("should fail: %s\n", mnemonic);
            if (!(t_flags & FCML_TSF_PRINT_ONLY)) {
                success = FCML_FALSE;
            }
            ...
            for (int j = 0; j < 2; j++) {
                ...
                while (assembled_instruction) {
                    assembled_code_len[assembled_code_index] =
                            assembled_instruction->code_length;
                    assembled_code[assembled_code_index++] =
                ...
                if (!(t_flags & FCML_TSF_PRINT_ONLY)) {
                    if (!(t_flags & FCML_TSF_MULTI_ASM_RESULTS)
                            && asm_result.number_of_instructions != 1) {
                        found = FCML_FALSE;
                        break;
                    }
                    /* Check if number of assembled instructions match.*/
                    if ((t_flags & FCML_TSF_MULTI_ASM_RESULTS)
                            && asm_result.number_of_instructions != code[0]) {
                        found = FCML_FALSE;
                        break;
                    }
                }
                ...
                while (instructions) {
                    ...
                    if ((t_flags & FCML_TSF_PRINT_ONLY)
                            || (differ && !looking_for_instruction)) {
#ifdef FCML_DEBUG
                        printf("Index: %d\nOriginal code : ",
                                assembled_instruction->__def_index);
#else
                        ...
                        for (i = 0;
                                i
                                        < (fcml_int) assembled_instruction->code_length;
                                i++) {
            ...
            /* Prints test code.*/
            if (!(t_flags & FCML_TSF_SHOULD_FAIL) && assembled_code_index >= 1
                    && ((t_flags & FCML_TSF_PRINT_ONLY) || !found)) {

                /* Mnemonic.*/
                fcml_string macro;
                switch (op_mode) {
                case FCML_OM_16_BIT:
                    macro = (assembled_code_index > 1) ?
                            (is_gas ? "FCML_A16_M" : "FCML_I16_M") :
                            (is_gas ? "FCML_A16" : "FCML_I16");
                    break;
                case FCML_OM_32_BIT:
                    macro = (assembled_code_index > 1) ?
                            (is_gas ? "FCML_A32_M" : "FCML_I32_M") :
                            (is_gas ? "FCML_A32" : "FCML_I32");
                    break;
                case FCML_OM_64_BIT:
                    macro = (assembled_code_index > 1) ?
                            (is_gas ? "FCML_A64_M" : "FCML_I64_M") :
                            (is_gas ? "FCML_A64" : "FCML_I64");
                    break;
                }
...
fcml_bool fcml_fn_ts_instruction_test_diss(fcml_uint8_t *code, fcml_int size,
        fcml_en_operating_mode op_mode, fcml_string mnemonic,
        fcml_uint32_t t_flags, fcml_uint32_t ren_flags) {
    ...
    fcml_st_disassembler_context context = { 0 };
    context.configuration.enable_error_messages = FCML_TRUE;
    context.configuration.short_forms =
            (t_flags & FCML_TSF_SHORT) ? FCML_TRUE : FCML_FALSE;
    context.configuration.extend_disp_to_asa = FCML_TRUE;
    context.configuration.conditional_group = FCML_DASM_CONDITIONAL_GROUP_1;
    context.configuration.carry_flag_conditional_suffix = FCML_TRUE;
    context.disassembler = disassembler;
    ...
    if (!error) {
        ...
        config.render_flags = FCML_REND_FLAG_HEX_IMM
                | FCML_REND_FLAG_COND_SHOW_CARRY
                | FCML_REND_FLAG_HEX_DISPLACEMENT | ren_flags;
        ...
        if (t_flags & FCML_TSF_PRINT_ONLY) {
            ...
            if (error || strcmp(buffer, mnemonic) != 0) {
                ...
                if (dis_result.instruction_details.instruction_size != size) {
                    printf(
                            "Instruction size: %d Disassembled code size: %d (%s)\n",
                            (fcml_uint32_t) dis_result.instruction_details.instruction_size,
                            size, mnemonic);
                    success = FCML_FALSE;
                } else {
                    if (!(t_flags & FCML_TSF_PRINT_ONLY)) {
