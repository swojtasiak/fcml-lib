...
static fcml_bool suite_init(void) {
    return FCML_TRUE;
}

static fcml_bool suite_cleanup(void) {
    return FCML_TRUE;
}
...
void fcml_tf_assembler_db_imm8(void) {
    ...
    if ((error = fcml_fn_dialect_init_intel( FCML_INTEL_DIALECT_CF_DEFAULT,
            &dialect))) {
...
void fcml_tf_assembler_byte_imm8(void) {
    ...
    /* Initializes the Intel dialect instance. */
    fcml_st_dialect *dialect;
    if ((error = fcml_fn_dialect_init_gas( FCML_GAS_DIALECT_CF_DEFAULT,
            &dialect))) {
        STF_FAIL("Can not initialize Intel dialect.");
        return;
    }
...
fcml_stf_test_suite fcml_si_assembler = {
    "suite-fcml-assembler", suite_init, suite_cleanup, fcml_ti_assembler
};
