...
void fcml_tf_parser_int_parse_test_decorator_opmask_k_z_for_addressing(void) {
    fcml_st_parser_result result;
    fcml_fn_parser_result_prepare(&result);
    STF_ASSERT_EQUAL(
            fcml_fn_parse(&internal_intel_context, "vaddpd cs:[rax+rbx]{k4}{z}",
                    &result), FCML_CEH_GEC_NO_ERROR);
    if (result.instruction != NULL) {
        fcml_st_operand *operand = &(result.instruction->operands[0]);
...
void fcml_tf_parser_int_parse_test_decorator_z(void) {
    fcml_st_parser_result result;
    fcml_fn_parser_result_prepare(&result);
    STF_ASSERT_EQUAL(
            fcml_fn_parse(&internal_intel_context, "vaddpd zmm31{z}",
                    &result), FCML_CEH_GEC_NO_ERROR);
    if (result.instruction != NULL) {
        fcml_st_operand *operand = &(result.instruction->operands[0]);
        STF_ASSERT_EQUAL(operand->decorators.z, FCML_TRUE);
...
fcml_stf_test_case fcml_ti_parser[] = {
    ...
    { "fcml_tf_parser_int_parse_test_decorator_opmask_k",
            fcml_tf_parser_int_parse_test_decorator_opmask_k },
    { "fcml_tf_parser_int_parse_test_decorator_z",
            fcml_tf_parser_int_parse_test_decorator_z },
    { "fcml_tf_parser_int_parse_test_decorator_opmask_k_z_for_addressing",
            fcml_tf_parser_int_parse_test_decorator_opmask_k_z_for_addressing },
